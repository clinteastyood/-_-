...
export default function ResultsTable({ project, days, workers }: ResultsTableProps) {
  ...
  const handleDownloadExcel = () => {
    ...
    try {
      ...
      // 파일 다운로드
      XLSX.writeFile(workbook, `${project.name}_${project.formattedMonth}_급여계산결과.xlsx`);
    } catch (error) {
    ...
    <Card className="bg-white shadow rounded-lg overflow-hidden">
      <CardHeader className="px-6 py-5 border-b border-neutral-100">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center">
          <div>
            <h2 className="text-lg font-medium text-neutral-500">급여 계산 결과</h2>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-neutral-100">
          <thead className="bg-neutral-50">
            <tr>
              <th scope="col" className="px-3 py-3 text-left text-xs font-medium text-neutral-400 uppercase tracking-wider whitespace-nowrap sticky left-0 bg-neutral-50 z-10 border-r border-neutral-100">
                이름
              </th>
              <th scope="col" className="px-3 py-3 text-left text-xs font-medium text-neutral-400 uppercase tracking-wider whitespace-nowrap">
                주민번호
              </th>
              <th scope="col" className="px-3 py-3 text-left text-xs font-medium text-neutral-400 uppercase tracking-wider whitespace-nowrap">
                임금 유형
              ...
              {days.map(day => (
                ...
                >
                  {project.formattedMonth.replace("년 ", "/").replace("월", "")}/{day}
                </th>
              ))}
              
              <th scope="col" className="px-3 py-3 text-center text-xs font-medium text-neutral-400 uppercase tracking-wider whitespace-nowrap">
                총 근무시간
              </th>
              <th scope="col" className="px-3 py-3 text-center text-xs font-medium text-neutral-400 uppercase tracking-wider whitespace-nowrap">
                기본급
              ...
              <th scope="col" className="px-3 py-3 text-center text-xs font-medium text-neutral-400 uppercase tracking-wider whitespace-nowrap bg-primary bg-opacity-10 border-l border-r border-primary-light">
                최종 급여
              </th>
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-neutral-100">
            {workers.map(worker => (
              <tr key={worker.id}>
                ...
                {/* 일자별 근무시간 */}
                {days.map(day => (
                  <td key={day} className="px-3 py-4 text-sm text-neutral-500 text-center">
      ...
      <div className="bg-neutral-50 px-6 py-4 border-t border-neutral-100">
        ...
        <ul className="mt-2 text-xs text-neutral-400 space-y-1">
          <li>• 연장근로: 1일 8시간 또는 1주 40시간 초과 시 통상임금의 50% 가산</li>
          <li>• 야간근로: 오후 10시부터 오전 6시까지 근로 시 통상임금의 50% 가산</li>
          <li>• 주휴수당: 주 15시간 이상 근무한 근로자에게 1일분의 임금 지급</li>
        </ul>
      </div>
    </Card>
  );
